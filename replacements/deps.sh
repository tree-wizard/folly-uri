...
mkdir -p _build
cd _build
CXX=clang++-7 CXXFLAGS='-ggdb -fsanitize=fuzzer-no-link' cmake configure .. -DFOLLY_ASAN_ENABLED=1 -DFOLLY_USE_SYMBOLIZER=1 -DBUILD_SHARED_LIBS=ON -DCMAKE_POSITION_INDEPENDENT_CODE=ON
